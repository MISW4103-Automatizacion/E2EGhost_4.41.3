...
When('Escenario22 Doy click en member', async function() {
    let element = await this.driver.$("(//a[@href='#/members/'])[1]");
    return await element.click();
})

When('Escenario22 Doy click en newMember', async function() {
    let element = await this.driver.$("//span[contains(.,'New member')]");
    return await element.click();
})

When('Escenario22 Escribo el nombre', async function () {
    let element = await this.driver.$('#member-name');
    return await element.setValue(name);
});


When('Escenario22 Escribo el nuevo nombre', async function () {
    let element = await this.driver.$('#member-name');
    return await element.setValue(new_name);
});

When('Escenario22 Escribo el nuevo email', async function () {
    let element = await this.driver.$('#member-email');
    return await element.setValue(new_mail);
});

When('Escenario22 Escribo una nueva nota', async function () {
    let element = await this.driver.$('#member-note');
    return await element.setValue(new_note);
});


When('Escenario22 Escribo el email', async function () {
    let element = await this.driver.$('#member-email');
    return await element.setValue(mail);
});

When('Escenario22 Doy click en Label', async function() {
    let element = await this.driver.$("(//div[@tabindex='0'])[2]");
    return await element.click();
});

When('Escenario22 Escribo el Label', async function () {
    let element = await await this.driver.$("(//div[@tabindex='0'])[2]");
    return await element.setValue(label);
});

When('Escenario22 Escribo una nota', async function () {
    let element = await this.driver.$('#member-note');
    return await element.setValue(note);
});

When('Escenario22 Doy click en save', async function() {
...
Then("Escenario22 Debe aparecer el Member en la lista y lo selecciono", async function () {
    isMostrar = true;
    let i = 2;
    let memberSave;
...
When('Escenario22 Doy click en boton settings', async function() {
    let element = await this.driver.$("/html/body/div[2]/div/main/section/div[1]/header/section/span/button");
    return await element.click();
});


When('Escenario22 Doy click en Delete Member', async function() {
...
When('Escenario22 Doy click en unsubscribe', async function() {
