...
When('Escenario24 Doy click en member', async function() {
    let element = await this.driver.$("(//a[@href='#/members/'])[1]");
    return await element.click();
})

When('Escenario24 Doy click en newMember', async function() {
    let element = await this.driver.$("//span[contains(.,'New member')]");
    return await element.click();
})

When('Escenario24 Escribo el nombre', async function () {
    let element = await this.driver.$('#member-name');
    return await element.setValue(name);
});


When('Escenario24 Escribo el nuevo nombre', async function () {
    let element = await this.driver.$('#member-name');
    return await element.setValue(new_name);
});

When('Escenario24 Escribo el nuevo email', async function () {
    let element = await this.driver.$('#member-email');
    return await element.setValue(new_mail);
});

When('Escenario24 Escribo una nueva nota', async function () {
    let element = await this.driver.$('#member-note');
    return await element.setValue(new_note);
});


When('Escenario24 Escribo el email', async function () {
    let element = await this.driver.$('#member-email');
    return await element.setValue(mail);
});

When('Escenario24 Doy click en Label', async function() {
    let element = await this.driver.$("(//div[@tabindex='0'])[2]");
    return await element.click();
});

When('Escenario24 Escribo el Label', async function () {
    let element = await await this.driver.$("(//div[@tabindex='0'])[2]");
    return await element.setValue(label);
});

When('Escenario24 Escribo una nota', async function () {
    let element = await this.driver.$('#member-note');
    return await element.setValue(note);
});

When('Escenario24 Doy click en save', async function() {
    let element = await this.driver.$("//span[contains(.,'Save')]");


    let body = await this.driver.$('/html/body');
    await body.saveScreenshot('reports/Ghost_' + json.VersionEnPrueba + '_Escenario24_crear_member_save(1).png');
...
Then("Escenario24 Debe aparecer el Member en la lista y lo selecciono", async function () {
    isMostrar = true;
    let i = 2;
    let memberSave;
    ...
    let body = await this.driver.$('/html/body');
    await body.saveScreenshot('reports/Ghost_' + json.VersionEnPrueba + '_Escenario24_crear_member(1).png');
     

  });


When('Escenario24 Doy click en boton settings', async function() {
    let element = await this.driver.$("/html/body/div[2]/div/main/section/div[1]/header/section/span/button");
    return await element.click();
});


When('Escenario24 Doy click en Delete Member', async function() {
...
When('Escenario24 Doy click en Confirm Delete Member', async function() {
    ...
    await body.saveScreenshot('reports/Ghost_' + json.VersionEnPrueba + '_Escenario24_delete_member(1).png');
    let element = await this.driver.$("(//span[contains(.,'Delete member')])[3]");
    return await element.click();
});

When('Escenario24 Doy click en unsubscribe', async function() {
...
if(json.VersionEnPrueba==2){
    ...
    When('Escenario24 Doy click en save', async function() {
        ...
        let body = await this.driver.$('/html/body');
        await body.saveScreenshot('reports/Ghost_' + json.VersionEnPrueba + '_Escenario24_crear_member_save(2).png');
    ...
    Then("Escenario24 Debe aparecer el Member en la lista y lo selecciono", async function () {
        ...
        await elementoEliminar.click();  
        let body = await this.driver.$('/html/body');
        await body.saveScreenshot('reports/Ghost_' + json.VersionEnPrueba + '_Escenario24_crear_member(2).png');
    ...
    When('Escenario24 Doy click en Confirm Delete Member', async function() {
        ...
        await body.saveScreenshot('reports/Ghost_' + json.VersionEnPrueba + '_Escenario24_delete_member(2).png');
