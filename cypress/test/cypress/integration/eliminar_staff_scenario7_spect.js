describe('Escenario 7 Ingresar a la aplicación, si el usuario no existe se crea, se dirige hasta el modulo de staff. Crear un staff de tipo Editor y validar que aparezca en la lista de staff, eliminarlos y validar que ya no aparezca en la lista, sale de la aplicación', () => {
      ...
      cy.get('main').then(($main) => {
        if($main.find('form').length > 0){
          if($main.find('form')[0].id == 'setup') {            
            ...
            cy.screenshot('Ghost_' + Cypress.env('VersionEnPrueba') + '_Escenario7_registrarUsuario_')
            salirAplicacion.salirAplicacion(cy)
          }
        }
      })      
      loginUser.loginUser(cy, Cypress.env('USER'), Cypress.env('PASSWORD'))
      cy.screenshot('Ghost_' + Cypress.env('VersionEnPrueba') + '_Escenario7_ingresoLogin_')
    })
    
    it('Eliminar un staff de tipo Editor', () => {
      if(Cypress.env('isRegresionVisual') == false){
        mail = faker.internet.email();
      } else {
        mail = 'pruebaRegresion@regresion.com.co';
      }
      cy.screenshot('Ghost_' + Cypress.env('VersionEnPrueba') + '_Escenario7_crear_staff_')
