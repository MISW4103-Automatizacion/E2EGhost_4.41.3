describe('Escenario 5 Ingresar a la aplicación, si el usuario no existe se crea, se dirige hasta el modulo de staff. Crear un staff de tipo Contributor y validar que aparezca en la lista de staff, eliminarlos y validar que ya no aparezca en la lista, sale de la aplicación', () => {
      ...
      cy.get('main').then(($main) => {
        if($main.find('form').length > 0){
          if($main.find('form')[0].id == 'setup') {            
            ...
            cy.screenshot('Ghost_' + Cypress.env('VersionEnPrueba') + '_Escenario5_registrarUsuario_')
            salirAplicacion.salirAplicacion(cy)
          }
        }
      })      
      loginUser.loginUser(cy, Cypress.env('USER'), Cypress.env('PASSWORD'))
      cy.screenshot('Ghost_' + Cypress.env('VersionEnPrueba') + '_Escenario5_ingresoLogin_')
    })
    ...
    it('Eliminar un staff de tipo Contributor', () => {
      ...
      cy.screenshot('Ghost_' + Cypress.env('VersionEnPrueba') + '_Escenario5_crear_staff_')
